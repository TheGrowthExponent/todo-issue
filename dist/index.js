...
import { generateTodoKey, renderIssueBody, findExistingIssue, createIssue, updateIssue, reopenIssue, } from './issueSync.js';
/**
...
async function run() {
    try {
        ...
        filesToScan = filesToScan.filter((f) => !micromatch.isMatch(f, config.scan.ignore || []));
        ...
        for (const todo of todos) {
            ...
            const issueBody = renderIssueBody(todo, {
                branch,
                commit,
            ...
            if (!existing) {
                // Create new issue
                await createIssue(octokit, repo, todo, { labels: issueLabels, assignees, milestone }, title, issueBody);
                issuesCreated++;
